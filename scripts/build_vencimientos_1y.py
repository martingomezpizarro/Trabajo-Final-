"""
build_vencimientos_1y.py

Serie histórica de vencimientos a 1 año por Tipo de Deuda y Moneda (SIGADE .mdb).
Cobertura: 2007-2025 (todos los archivos .mdb disponibles).

Idéntico a build_vencimientos_2y.py salvo la ventana temporal: aquí se suman
los servicios (principal + interés) con Fecha de Servicio dentro del año
posterior a la fecha de cada base, es decir [file_date, file_date + 1 año].
...
Output: data/Variables Finales/vencimientos_1y_nuevo.xlsx
  Hoja "Por Tipo y Moneda" — estructura jerárquica:
      ...
      <Tipo de Deuda 2>
        ...
  Hoja "Resumen" — totales por tipo (sin desglose moneda)
"""
...
OUTPUT_PATH = Path(r"c:\Users\Usuario\Desktop\MARTIN\ECONOMICS\TRABAJO FINAL\Trabajo\data\Variables Finales\vencimientos_1y_nuevo.xlsx")

# Ventana de vencimientos (en años) hacia adelante desde la fecha de cada base
WINDOW_YEARS = 1
...
def read_vencimientos(mdb_path: Path, file_date: date) -> pd.DataFrame | None:
    ...
    filtered to the 1-year window [file_date, file_date + 1 year].
    """
    ...
    # Filter: 1-year window
    cutoff = pd.Timestamp(file_date) + pd.DateOffset(years=WINDOW_YEARS)
...
def write_detail_sheet(ws, all_tipos: list[str], all_cols: list[str],
    ...
    ws.cell(2, 1, "VENCIMIENTOS A 1 AÑO POR TIPO DE DEUDA Y MONEDA")
...
def write_resumen_sheet(ws, all_tipos: list[str], all_cols: list[str],
    ...
    ws.cell(1, 1, "MINISTERIO DE ECONOMIA — SECRETARIA DE FINANZAS")
    ws.cell(2, 1, "VENCIMIENTOS A 1 AÑO POR TIPO DE DEUDA")
    ws.cell(3, 1, "SERIE HISTORICA SIGADE 2007-2025 | Millones de USD")
